-- Filtros novos na vitrine: Faixa de Ano e Fonte (OLX/Webmotors/Mercado Livre).
-- Recria oportunidades_por_proximidade (0018) com p_ano_min/p_ano_max/p_fonte,
-- mesmo princípio dos demais (null = não filtra). `ano` é text, mas comparação
-- lexicográfica de anos de 4 dígitos = comparação numérica.

-- Precisa dropar a assinatura antiga antes (adicionar params cria overload).
drop function if exists oportunidades_por_proximidade(
  double precision, double precision, text, text, text, numeric, numeric, boolean, int, int
);
