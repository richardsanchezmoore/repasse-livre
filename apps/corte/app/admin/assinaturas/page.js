...
export default async function AdminAssinaturasPage() {
  ...
  const webhook = `${proto}://${host}/api/cakto?secret=${process.env.CORTE_CAKTO_SECRET || "DEFINA_CORTE_CAKTO_SECRET"}`;
  ...
  return (
    <main className="screen">
      ...
      <div style={{ marginTop: 16 }}>
        <ConstrutorAssinaturas planosIniciais={planos} webhook={webhook} />
      </div>
    </main>
  );
}
