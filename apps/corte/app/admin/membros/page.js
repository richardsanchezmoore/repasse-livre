...
export default async function AdminMembrosPage() {
  ...
  const [{ data: membros }, { data: acessos }, { data: dossies }, { data: eventos }] = await Promise.all([
    ...
    admin.from("corte_eventos").select("user_id").eq("tipo", "pdf_baixado"),
  ]);
  ...
  const pdfCount = {};
  for (const e of eventos || []) pdfCount[e.user_id] = (pdfCount[e.user_id] || 0) + 1;
