...
import { resolverReferenciaFipePorValorHibrido, separarMarcaModelo } from "./fipeService.js";
...
async function resolverCodigo(
  ...
  const { marca, modelo } = separarMarcaModelo(veiculo);
  const ref = await resolverReferenciaFipePorValorHibrido(marca, modelo, ano, versao, valorPagina).catch(() => null);
  if (!ref) return null;
