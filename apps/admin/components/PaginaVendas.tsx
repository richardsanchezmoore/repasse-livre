...
export interface DadosVendas {
  ...
  /** Checkout hospedado SEM `sck` — a página é estática, então o `sck` do logado é
   *  anexado no cliente pelo AcaoAssinatura (modo "auto"). */
  checkoutUrl: string | null;
  gerenciarUrl: string | null;
  whatsappSuporte: string | null;
  gateway: string | null;
}
// REMOVIDOS (eram o que prendia a /planos no server-render a cada visita):
//  · `estado`  → o AcaoAssinatura descobre no cliente (modo "auto").
//  · `aviso`   → o AvisoAssinatura lê da URL no cliente.
//  · `jaPremium` → era CÓDIGO MORTO: as duas páginas preenchiam, nada lia.
...
export function PaginaVendas({ dados }: { dados: DadosVendas }) {
  ...
  return (
    <div style={{ width: "100%", overflowX: "clip", background: "#EEF1F4" }}>
      ...
      {/* Retorno do checkout: lido da URL no CLIENTE (a página é estática/ISR). O
          Suspense é exigência do useSearchParams em rota estática. */}
