                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 10, margin: "0 0 16px" }}>
                  {dados.numerosRegionais.map((n) => (
                    <div key={n.uf} style={{ flex: "1 1 150px", background: "#F4FBF6", border: "1px solid #D8EEDF", borderRadius: 14, padding: "12px 14px" }}>
                      ...
                      <div style={{ font: `600 12.5px ${CORPO}`, color: "#3a4652", marginTop: 3 }}>carros abaixo da FIPE {n.preposicao} <b style={{ color: "#1f2937" }}>{n.nome}</b></div>
                      {n.novas24h > 0 && (
                        <div style={{ display: "flex", alignItems: "center", gap: 4, font: `600 11.5px ${CORPO}`, color: "#2f6446", marginTop: 5 }}>
                          <Clock size={12} strokeWidth={2.2} />
                          <span><b style={{ fontSize: 14 }}>+{nfMilhar(n.novas24h)}</b> nas últimas 24h</span>
                        </div>
                      )}
                    </div>
                  ))}
